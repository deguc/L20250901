...
class Linear:
    ...
    def __init__(self,d_in,d_out,scale=1):

        self.ps = [
            np.random.rand(d_in,d_out)/scale,
            np.zeros(d_out)
        ]
...
layers1 = [
    Linear(d_in,d_h,scale=1),
    Sigmoid(),
    Linear(d_h,d_out)
]
layers2 = [
    Linear(d_in,d_h,scale=xavier),
    Sigmoid(),
    Linear(d_h,d_out)
]
layers3 = [
...
loss4=Model(data,layers4,epochs=epochs,lr=lr,batch_size=batch_size)
...
ax1.plot(loss1,color='blue',label='Normal')
ax1.plot(loss2,color='red',label='Xavier')

ax2 = fig.add_subplot(122)
ax2.set_title('ReLU')
ax1.set_xlabel('epochs')
ax1.set_ylabel('cross entropy')
ax2.plot(loss3,color='blue',label='Normal')
ax2.plot(loss4,color='red',label='He')
